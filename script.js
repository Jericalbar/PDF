...
// This is the REAL export/print page. Anything outside this rectangle is
// clipped from PNG/PDF/SVG/Word/Print output. Editing on the canvas is unchanged.
// The default matches the wide paper area shown in the current flowchart.
const PAPER = { x: 0, y: 0, w: 1200, h: 560 };

function paperBounds() {
  return { minX: PAPER.x, minY: PAPER.y, w: PAPER.w, h: PAPER.h, maxX: PAPER.x + PAPER.w, maxY: PAPER.y + PAPER.h };
}
...
function exportSvg() {
  ...
  status("SVG exported (paper clipped)");
}
...
function buildExportCanvas(scale = 2) {
  ...
  // HARD PAPER CLIP: nothing drawn here can appear outside the page.
  ctx.save();
  ctx.beginPath();
...
function exportPng() {
  ...
  c.toBlob(blob => { download("flowchart.png", blob); status("PNG exported (paper clipped)"); });
}
...
function printFlowchart() {
  ...
  printWin.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Flowchart Print</title><style>
    @page { size: landscape; margin: 0; }
    html,body { margin:0; padding:0; background:#fff; width:${PAPER.w}px; height:${PAPER.h}px; overflow:hidden; }
    .page { width:${PAPER.w}px; height:${PAPER.h}px; overflow:hidden; position:relative; }
    img { display:block; width:${PAPER.w}px; height:${PAPER.h}px; object-fit:fill; }
  </style></head><body><div class="page"><img src="${imgData}" /></div><script>window.onload=()=>{setTimeout(()=>window.print(),120);};window.onafterprint=()=>window.close();<\/script></body></html>`);
...
<body style="margin:0"><img src="${imgData}" style="width:${PAPER.w}px;height:${PAPER.h}px;display:block" /></body></html>`;
  download("flowchart.doc", new Blob(["\ufeff", html], { type: "application/msword" }));
  status("Word exported (paper clipped)");
}
...
// Ctrl+P prints only the fixed paper area, never the overflowing editor canvas.
document.addEventListener("keydown", e => {
